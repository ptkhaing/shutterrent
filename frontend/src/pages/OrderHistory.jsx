...
function OrderHistory() {
  ...
  return (
    <div className="p-8 py-12 relative max-w-3xl mx-auto">
      <h2 className="font-display text-2xl sm:text-3xl mb-8 text-center text-ink-800 dark:text-ink-100">My Booking History</h2>
      {loading ? (
        <p className="text-center text-ink-400 dark:text-ink-500">Loading...</p>
      ) : bookings.length === 0 ? (
        <p className="text-center text-ink-400 dark:text-ink-500">You have no past bookings.</p>
      ) : (
        <div className="space-y-4">
          {bookings.map((booking) => (
            <div key={booking._id} className="border border-ink-200 dark:border-ink-700 rounded-lg p-5 bg-white dark:bg-ink-800">
              <h3 className="text-lg font-semibold mb-2 text-ink-800 dark:text-ink-100">{booking.listing?.title}</h3>
              ...
              <p className="text-ink-500 dark:text-ink-400 text-sm mt-2">{booking.listing?.description}</p>
              <p className="text-ink-800 dark:text-ink-100 font-semibold mt-2">{booking.listing?.pricePerDay.toLocaleString()} Ks per day</p>
      ...
      {confirmModal.visible && (
        <div className="fixed inset-0 bg-ink-900/60 flex items-center justify-center z-50 px-4">
          <div className="bg-white dark:bg-ink-800 p-6 rounded-lg shadow-lg w-full max-w-sm text-center">
            <h2 className="font-display text-lg mb-4 text-ink-800 dark:text-ink-100">Cancel Booking?</h2>
            <p className="text-ink-600 dark:text-ink-300 mb-6 text-sm">Are you sure you want to cancel this booking?</p>
            <div className="flex justify-center gap-4">
              <button
                className="px-4 py-2 bg-ink-100 dark:bg-ink-700 text-ink-700 dark:text-ink-200 rounded-full hover:bg-ink-200 dark:hover:bg-ink-600 transition-colors duration-200"
